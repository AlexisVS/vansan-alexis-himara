  ...
  <div class="py-12">
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
      <section>
        ...
        <x-form.form action='/dashboard/blog/{{ $edit->id }}' method='PUT' enctype title='Edition team member {{ $edit->details_name }}' subtitle="All text in your page">
          <x-form.upload name="figure_link_img" />
          <x-form.text name="title_text" :text="$edit->title_text" />
          <x-form.textarea name="text" :text="$edit->text" />
          <x-form.upload name="author_img" />
          <x-form.text name="author_text" :text="$edit->author_text" />
          <x-form.fa-select name="date_i_class" title="" :icons="$icons" selected="{{ $edit->date_i_class }}" />
          <x-form.text name="date_text" :text="$edit->date_text" />
          <x-form.fa-select name="comments_i_class" title="" :icons="$icons" selected="{{ $edit->date_i_class }}" />
          <x-form.text name="comments_text" :text="$edit->comments_text" />
            <div class="col-start-1 col-span-full md:col-start-2 md:col-span-4">
              ...
              <select name="post_category_id"
                ...
                @foreach($categories as $option)
                  <option 
                    value="{{ $option->id }}" 
                    @if ($option->id == $edit->post_category_id) 
                      selected="selected"
                    @endif>
                    {{ $option->name }}
                  </option>
