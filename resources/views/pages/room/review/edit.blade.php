<x-app-layout>
  <x-slot name="header">
    <div class="flex items-center space-x-12">
      ...
      <h2 class="font-semibold font-display text-xl text-himaraGold-500 leading-tight">
        Edit room review by {{ $edit->review_info }}
      </h2>
    </div>
  ...
  <div class="py-12">
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
      ...
      @endphp
        <x-form.form action='/dashboard/room/{{ $roomId }}/review/{{ $edit->id }}' method='PUT' enctype title='Edition review {{ $edit->details_name }}'
          ...
          <x-form.text name="review_info" :text="$edit->review_info" />
          <x-form.text name="review_date" :text="$edit->review_date" />
          <x-form.text name="review_location" :text="$edit->review_location" />
          <x-form.textarea name="review_text" :text="$edit->review_text" />
        </x-form.form>
