<x-app-layout>
  <x-slot name="header">
    <div class="flex items-center justify-between space-x-4 w-full">
      ...
      <a href="/dashboard/room/category/create" class="px-6 py-3 mr-2 mt-3 bg-himaraBlue-500 text-white shadow rounded-md hover:bg-himaraBlue-600">Add category</a>
    </div>
  ...
  <div class="py-12">
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
      <section>
        ...
        <x-table.table :columns="collect($index->first())->keys()" crud-uri="/dashboard/room/category/" :data-tables="$index->take(4)" numberHeadActions="-1" show="" edit="" delete="" />
        @if ($index->count() > 4)
        <x-table.table :columns="collect($index->first())->keys()" crud-uri="/dashboard/room/category/" :data-tables="$index->where('id', '>', 4)" numberHeadActions="1" show="" edit="" />
        @endif
      </section>
