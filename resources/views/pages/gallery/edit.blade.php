<x-app-layout>
  <x-slot name="header">
    <div class="flex items-center space-x-12">
      ...
      <h2 class="font-semibold font-display text-xl text-himaraGold-500 leading-tight">
        Gallery Elements {{ $edit->figcaption }}
      </h2>
    </div>
  ...
  <div class="py-12">
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
      <section>
        <x-form.form action='/dashboard/gallery/{{ $edit->id }}' method='PUT' enctype title='Edition gallery elements'
          subtitle="All text in your page">
          <x-form.select name="class_filter" multiple :options="$gallery_filters->pluck('name')->toArray()"
            selected="{{ $edit->class_filter }}" />
          <x-form.upload name="img" />
          <x-form.text name="figcaption" :text="$edit->figcaption" :text="$edit->figcaption" />
        </x-form.form>
